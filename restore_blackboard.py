...
start_idx = content.find("function TutorialPlayer({")
end_idx = content.find("export default function UltimateLearnEngine() {")

if start_idx == -1 or end_idx == -1:
    print("Could not find boundaries!")
    exit(1)

# we need to keep the export default function UltimateLearnEngine() {
new_tutorial_player = """function TutorialPlayer({
  lesson,
  onClose,
  onComplete,
  studentName,
  ...
  return (
    <div className="fixed inset-0 z-[200] flex flex-col overflow-hidden" style={{ background: boardBg }}>
      ...
      <div className="relative z-10 flex items-center justify-between px-3 sm:px-8 py-2.5 sm:py-3" style={{ borderBottom: '2px solid rgba(90,122,90,0.3)' }}>
        ...
        <button onClick={() => step && audioEngine?.speak(step.speak)}
      ...
      <div className="relative z-10 flex-1 flex flex-col items-center justify-center px-3 sm:px-8 pb-4 select-none overflow-hidden"
        ...
        {/* Guide row */}
        {!completed && !quizPhase && !isTraceStep && step && (
...
content = content[:start_idx] + new_tutorial_player + "\n\n" + content[end_idx:]
with codecs.open(path, 'w', 'utf-8') as f:
    f.write(content)
print("RESTORED BLACKBOARD!")
