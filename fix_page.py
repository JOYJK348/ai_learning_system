...
try:
    ...
    for i, line in enumerate(lines):
        if '{renderGuideOwl()}' in line and start_idx == -1:
            # We want to replace from line 596 (which is '{renderGuideOwl()}' inside the broken part)
            start_idx = i - 1  # actually let's just do a specific index
            
    # the broken section in the file starts around line 596
    # Let's just find the exact snippet:
    # 596:             {renderGuideOwl()}
    # 597:               Back
    # 598:             </button>
    ...
    with open(path, 'w', encoding='utf-8') as f:
        for i, line in enumerate(lines):
            if i == 595: # Before {renderGuideOwl()}
                f.write(line)
                
                # Write the whole massive block
                f.write('''        <button onClick={() => audioEngine?.speak(step.speak)}
...
''')
                # Now skip until we find '</div>' for 'Bottom controls' and then return the rest
                # The previous corrupted file had:
                # 596:             {renderGuideOwl()}
                # 597:               Back
                # 598:             </button>
                # 599:             <button onClick={handleNext}
                # ...
                # 606:       )}
                # 607:     </div>
                # 608:   );
                # 609: });
            elif i > 595 and i <= 609:
