...
const LETTERS = [
  { letter: 'க', word: 'கடல்',   emoji: '🌊', hint: 'Sea',      color: 'bg-red-50 text-red-700 border-red-100' },
  { letter: 'ச', word: 'சட்டை',  emoji: '👕', hint: 'Shirt',    color: 'bg-orange-50 text-orange-700 border-orange-100' },
  { letter: 'ட', word: 'டமாரம்', emoji: '🥁', hint: 'Drum',     color: 'bg-amber-50 text-amber-700 border-amber-100' },
  { letter: 'த', word: 'தமிழ்',  emoji: '📚', hint: 'Tamil',    color: 'bg-yellow-50 text-yellow-700 border-yellow-100' },
  { letter: 'ந', word: 'நரி',    emoji: '🦊', hint: 'Fox',      color: 'bg-lime-50 text-lime-700 border-lime-100' },
  { letter: 'ப', word: 'பழம்',   emoji: '🍎', hint: 'Fruit',    color: 'bg-green-50 text-green-700 border-green-100' },
  { letter: 'ம', word: 'மரம்',   emoji: '🌳', hint: 'Tree',     color: 'bg-teal-50 text-teal-700 border-teal-100' },
  { letter: 'ய', word: 'யானை',  emoji: '🐘', hint: 'Elephant', color: 'bg-sky-50 text-sky-700 border-sky-100' },
  { letter: 'ர', word: 'ரோஜா',  emoji: '🌹', hint: 'Rose',     color: 'bg-blue-50 text-blue-700 border-blue-100' },
  { letter: 'ல', word: 'லட்டு',  emoji: '🍡', hint: 'Sweet',    color: 'bg-indigo-50 text-indigo-700 border-indigo-100' },
  { letter: 'வ', word: 'வண்டி',  emoji: '🚗', hint: 'Vehicle',  color: 'bg-violet-50 text-violet-700 border-violet-100' },
  { letter: 'ன', word: 'நன்றி',  emoji: '🙏', hint: 'Thanks',   color: 'bg-pink-50 text-pink-700 border-pink-100' },
];

const QUESTIONS = [
  { emoji: '🌊', word: 'கடல்',   hint: 'Sea',      target: 'க', options: ['க', 'ச', 'ம', 'ய'] },
  { emoji: '🐘', word: 'யானை',  hint: 'Elephant', target: 'ய', options: ['ர', 'ந', 'ய', 'ல'] },
  { emoji: '🌳', word: 'மரம்',   hint: 'Tree',     target: 'ம', options: ['ம', 'ட', 'த', 'வ'] },
  { emoji: '🍎', word: 'பழம்',   hint: 'Fruit',    target: 'ப', options: ['ப', 'க', 'ச', 'ர'] },
  { emoji: '🦊', word: 'நரி',    hint: 'Fox',      target: 'ந', options: ['ந', 'ல', 'ம', 'ட'] },
];

export default function TamilUyirmeiQuiz1({ onComplete }: Props) {
  ...
  if (done) return (
    <div className="flex flex-col items-center gap-5 px-6 py-10 kids-font">
      ...
      <h2 className="text-2xl font-black text-indigo-950 text-center">க முதல் ன — கற்றுவிட்டீர்கள்!</h2>
    ...
    <div className="flex flex-col items-center gap-4 px-3 py-2 w-full max-w-md mx-auto kids-font select-none">
      ...
      <div className="text-center">
        <span className="text-[10px] font-black text-indigo-900/40 uppercase tracking-widest px-3 py-1 bg-indigo-50 rounded-full border border-indigo-100/50">
          🔤 உயிர்மெய் எழுத்துக்கள் — க முதல் ன
        </span>
        <h3 className="text-xl font-black text-indigo-950 mt-1">
          {mode === 'learn' ? 'தொட்டு கற்கலாம்! 💡' : `"${QUESTIONS[qIdx].word}" எந்த எழுத்தில் தொடங்கும்?`}
        </h3>
      </div>
      ...
      {mode === 'learn' ? (
        <div className="w-full flex flex-col gap-4">
          <div className="grid grid-cols-6 gap-1.5">
            {LETTERS.map(item => (
              <button key={item.letter} onClick={() => setSelected(item)}
                className={`aspect-square rounded-xl border-2 flex flex-col items-center justify-center p-1 cursor-pointer shadow-sm ${item.color} ${selected?.letter === item.letter ? 'ring-4 ring-indigo-400 border-indigo-400' : ''}`}>
            ...
            {selected ? (
              <>
                <div className="flex items-center gap-3">
                  <span className="text-5xl font-black text-indigo-950 bg-indigo-50 px-4 py-2 rounded-2xl border border-indigo-100">{selected.letter}</span>
                  <div>
          ...
          <button onClick={() => setMode('quiz')}
            className="w-full py-3.5 bg-indigo-600 text-white rounded-2xl font-black text-base shadow-lg border-b-4 border-indigo-800 active:scale-95 cursor-pointer">
            விளையாட்டு தொடங்கு! 🎮
          </button>
        </div>
      ) : (
        <div className="w-full flex flex-col gap-4">
          <div className="flex items-center justify-center gap-1.5">
            {QUESTIONS.map((_, i) => (
              <div key={i} className={`w-3 h-3 rounded-full border ${i < qIdx ? 'bg-emerald-500 border-emerald-400' : i === qIdx ? 'bg-indigo-600 border-indigo-500 ring-2 ring-indigo-300' : 'bg-white border-slate-200'}`} />
            ))}
          </div>
          <div className="w-full bg-gradient-to-br from-indigo-50 to-violet-50 border-2 border-indigo-100 rounded-3xl p-6 flex flex-col items-center gap-2 min-h-[9rem]">
          ...
          <div className="grid grid-cols-4 gap-3">
            {shuffledOptions.map(opt => {
              ...
              let cls = 'bg-white text-indigo-950 border-slate-200 hover:bg-indigo-50';
              ...
              return (
                <button key={opt} onClick={() => handlePick(opt)} disabled={!!picked}
                  className={`aspect-square rounded-2xl border-2 flex items-center justify-center text-3xl font-black shadow-sm cursor-pointer active:scale-95 ${cls}`}>
                  {opt}
                </button>
              );
            })}
