...
const BUBBLE_POOL: LetterBubble[] = [
  { id: 'c1', char: 'B', isConsonant: true, exampleEmoji: '⚽', exampleWord: 'Ball' },
  { id: 'v1', char: 'A', isConsonant: false, exampleEmoji: '🍎', exampleWord: 'Apple' },
  { id: 'c2', char: 'C', isConsonant: true, exampleEmoji: '🐱', exampleWord: 'Cat' },
  { id: 'v2', char: 'E', isConsonant: false, exampleEmoji: '🥚', exampleWord: 'Egg' },
  { id: 'c3', char: 'D', isConsonant: true, exampleEmoji: '🐶', exampleWord: 'Dog' },
  { id: 'v3', char: 'I', isConsonant: false, exampleEmoji: '🍦', exampleWord: 'Ice Cream' },
  { id: 'c4', char: 'F', isConsonant: true, exampleEmoji: '🐟', exampleWord: 'Fish' },
  { id: 'v4', char: 'O', isConsonant: false, exampleEmoji: '🐙', exampleWord: 'Octopus' },
  { id: 'c5', char: 'G', isConsonant: true, exampleEmoji: '🍇', exampleWord: 'Grapes' },
];

export default function ConsonantCollectorQuiz({ onComplete }: Props) {
  ...
  const totalConsonants = useMemo(() => {
    return BUBBLE_POOL.filter(b => b.isConsonant).length;
  }, []);

  const handleBubbleTap = (bubble: LetterBubble) => {
    if (selectedIds.has(bubble.id) || wrongId || done) return;

    if (bubble.isConsonant) {
      const nextSelected = new Set(selectedIds);
      nextSelected.add(bubble.id);
      setSelectedIds(nextSelected);
      setScore(s => s + 1);

      if (nextSelected.size === totalConsonants) {
        setTimeout(() => {
          setDone(true);
        }, 1200);
      }
  ...
  if (done) {
    return (
      <div className="flex flex-col items-center gap-6 px-6 py-10 kids-font">
        ...
        <h2 className="text-3xl font-black text-indigo-950 text-center uppercase tracking-tight">Consonant Expert!</h2>
        <p className="text-lg font-bold text-indigo-900/60 text-center -mt-2">
          Awesome! You identified the consonant letters! 🌟
        </p>
        <button
          onClick={() => onComplete({
            score: 100, max_score: 100,
            completion_data: { score, total: totalConsonants },
    ...
    <div className="flex flex-col items-center gap-5 px-4 py-4 w-full max-w-sm mx-auto kids-font select-none">
      ...
      <div className="text-center">
        <span className="text-[10px] font-black text-indigo-900/40 uppercase tracking-widest px-3 py-1 bg-indigo-50 rounded-full border border-indigo-100/50">
          🧩 Consonant Collector Quest
        </span>
        <h3 className="text-2xl font-black text-indigo-950 mt-2">
          Find and tap the <span className="text-indigo-650">Consonants</span>!
        </h3>
      </div>
      ...
      <div className="flex items-center gap-2 bg-indigo-50/50 px-4 py-1.5 rounded-full border border-indigo-100/30">
        <span className="text-[10px] font-black text-indigo-950 uppercase tracking-widest flex items-center gap-1.5">
          Stars: {selectedIds.size} / {totalConsonants} <Star size={12} className="text-amber-500 fill-amber-500" />
        </span>
      </div>
