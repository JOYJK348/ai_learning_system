...
export default function SimpleSubtractionQuiz({ onComplete }: Props) {
  ...
  if (done) {
    return (
      <div className="flex flex-col items-center gap-6 px-6 py-10 kids-font">
        ...
        <h2 className="text-3xl font-black text-indigo-950 text-center uppercase tracking-tight">Subtraction Champ!</h2>
        <p className="text-lg font-bold text-indigo-900/60 text-center -mt-2">
          Spectacular! You solved all the subtraction equations! 🌟
        </p>
        <button
          onClick={() => onComplete({
  ...
  return (
    <div key={currentIdx} className="flex flex-col items-center gap-4 px-3 py-2 w-full max-w-sm mx-auto kids-font select-none">
      ...
      {/* Subtraction Equation & Visual Count Blocks */}
      <div className="w-full bg-[#fdfbf6] border-4 border-[#e9d1a8] rounded-[2rem] p-4 shadow-md flex flex-col items-center gap-3">
        ...
        <div className="flex items-center justify-center gap-4 py-1">
          {/* Num 1 Blocks with some crossed out */}
          <div className="flex flex-col items-center gap-1">
            <div className="flex gap-1">
              {Array.from({ length: currentQ.num1 }).map((_, i) => {
                const isCrossed = i >= currentQ.num1 - currentQ.num2;
                return (
                  <div key={i} className={`w-4 h-4 rounded border shadow-sm flex items-center justify-center text-[9px] font-bold leading-none
                    ${isCrossed ? 'bg-red-50 border-red-300 text-red-500' : 'bg-sky-500 border-sky-600 text-transparent'}`}
                  >
                    {isCrossed ? '×' : ''}
                  </div>
                );
      ...
      <div className={`w-full flex flex-col items-center justify-center rounded-2xl border-4 py-2 shadow-inner transition-all duration-200
        ${showCorrect 
          ? 'bg-emerald-555 border-emerald-600 text-white' 
          : showWrong 
            ? 'bg-red-555 border-red-650 text-white shake' 
        ...
        <span className="text-[9px] font-black opacity-60 tracking-wider uppercase">
          {showCorrect ? 'CORRECT!' : showWrong ? 'WRONG!' : 'WRITE DIFFERENCE HERE'}
        </span>
      ...
      <div className="min-h-[20px] flex items-center justify-center text-center">
        {showCorrect ? (
          ...
          <p className="text-[10px] font-bold text-indigo-950/40 uppercase tracking-widest flex items-center gap-1 select-none">
            <HelpCircle size={11} /> Count blocks without an &apos;×&apos; cross, then type the result!
          </p>
        )}
      </div>
    </div>
