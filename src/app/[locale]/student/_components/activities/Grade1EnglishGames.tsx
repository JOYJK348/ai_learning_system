...
/* ==========================================================================
   SIMPLE TRACING CANVAS — now powered by KidsTraceCanvas (big, mobile-first)
   ========================================================================== */
import { KidsTraceCanvas } from './KidsTraceCanvas';

export function SimpleTraceCanvas({ letter, onComplete, language }: {
  letter: string;
  onComplete: () => void;
  language?: 'tamil' | 'hindi' | 'english';
}) {
  // Auto-detect language from the letter script
  const detectedLang = language
    ?? (/[\u0B80-\u0BFF]/.test(letter) ? 'tamil'
      : /[\u0900-\u097F]/.test(letter) ? 'hindi'
      : 'english');
